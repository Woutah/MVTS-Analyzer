...
# class LimitedValue(QtCore.QObject):
class LimitedValue():
	...
	def min_val(self, value):
		if value != self._min_val:
			...
			self.val = max(self.min_val, self.val)
	...
	def max_val(self, value):
		if value != self._max_val:
			...
			self.val = min(self.max_val, self.val)
			# self.changed.emit(self)
...
class LimitedRange():
	...
	def min_val(self, value):
		if value != self._min_val:
			if self._enforce_limits:
				...
				self.left_val = max(self.min_val, self.left_val)
				self.right_val = max(self.min_val, self.right_val)
			else:
				self._min_val = value
			# self.changed.emit(self)
	...
	def max_val(self, value):
		if value != self._max_val:
			if self._enforce_limits:
				self._max_val = max(self.min_val, value)
				self.left_val = min(self.max_val, self.left_val)
				self.right_val = min(self.max_val, self.right_val)
				# self.changed.emit(self)
			else:
				self._max_val = value
	...
	def get_type(self):
		...
		if self._min_val is not None and not isinstance(self._min_val, curType):
			curType = type(self._min_val)
			changes += 1 
		if self._max_val is not None and not isinstance(self._max_val, curType):
			curType = type(self._max_val)
			changes += 1
		if self._left_val is not None and not isinstance(self._left_val, curType):
			curType = type(self._left_val)
			changes += 1
		if self._right_val is not None and not isinstance(self._right_val, curType):
			curType = type(self._right_val)
			changes += 1

		log.debug(f"Curtype of limrange is now{curType}")
	...
	def find_bounded(self, left_val=None, right_val=None, none_to_limit = True):
		...
		for key, val in final_values.items(): #Limit both by left and right
			if self._min_val is not None and val is not None:
				final_values[key] = max(self._min_val, final_values[key])
			if self._max_val is not None and val is not None:
				final_values[key] = min(self._max_val, final_values[key])
	...
	def copy_limits(self, limrange : LimitedRange):
		...
		a, b = self.find_bounded(self.left_val, self.right_val)
	...
	def find_factor(self, val) -> float:
